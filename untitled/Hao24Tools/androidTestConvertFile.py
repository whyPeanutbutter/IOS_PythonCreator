            ...
            if len(arrTemp) > 1:
                if "string" in arrTemp:
                    outStr = outStr + '\n' + "private String " + protertyName + ';'
                        ...
                        "integer" in arrTemp) or ("date" in arrTemp):
                    outStr = outStr + '\n' + "private Integer " + protertyName + ';'
                elif ("bool" in arrTemp) or ("boolean" in arrTemp):
                    outStr = outStr + '\n' + "private Boolean " + protertyName + ';'
                elif ("double" in arrTemp) or ("float" in arrTemp):
                    outStr = outStr + '\n' + "private Float " + protertyName + ';'
                elif arrTemp[1].startswith('list<'):
                    outStr = outStr + '\n' + "private List<EL" + orginArrTemp[1][5:-1] + ' ' + "> " + protertyName + ';(这是推测类型)'
                else:
                    outStr = outStr + '\n' + "private " + orginArrTemp[1] + ' ' + protertyName + ';(这是推测类型)'
            else:
                print(outStr)
                print(arrTemp)
                if arrTemp[0] == 'success':
                    ...
                    outStr = "public class EL" + protertyName[0:1].capitalize() + protertyName[1:] + ' { (这是推测类型)'
                else:
                    outStr = outStr + "\n}\n" + '\n\n' + "public class EL" + strTemp[0:1].capitalize() + strTemp[
                                                                                                          1:] + ' { (这是推测类型)'
